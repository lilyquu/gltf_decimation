...
impl Vertex {
    ...
    fn new_q(index:u32, x:f32, y:f32, z:f32, q_matrix:Vec<f32>, edge_set:Vec<Edge>, face_set:Vec<Face>) -> Self {
        Vertex {
            index,
            x,
            y,
    ...
    fn add_edge(&mut self, new_edge:Edge) {
        self.edge_set.push(new_edge);
    }

    // Adding face to face_set
    fn add_face(&mut self, new_face:Face) {
        self.face_set.push(new_face);
    }

    fn update_q_matrix(&mut self){
        ...
        for face in self.face_set.clone(){
            for i in 0..10 {
...
fn initialize(json:&Value, data:&Vec<Vec<DataType>>, mut vertex_list:Vec<Vertex>, mut edge_list:Vec<Edge>, mut face_list:Vec<Face>) {
    ...
    println!("data layer's length:{}, {}, {}, {}", data[0].len(), data[1].len(), data[2].len(), data[3].len());
    // iterate through triangles using index list
    for i in 0..index.len()-2 {
        // Initialize 3 vertices
        println!("position's index: {}", (i+2)*3+2);
        println!("index length: {}", index.len());
        println!("position length: {}", position.len());
        ...
        vertex_list.push(v_1);
        vertex_list.push(v_2);
        vertex_list.push(v_3);
        edge_list.push(e_1);
        edge_list.push(e_2);
        edge_list.push(e_3);
        face_list.push(f);
...
//        if unique_values.push(pair.clone()) {
//            result.push(pair);
//        }
//    }
//    
